                 ...
                 int32_t maxsearch, bool capend, bool allowpart, bool adjgoalbounds)
{
	...
	int32_t ps = 0;
	for(auto pit=pj->path->begin(); pit!=pj->path->end(); pit++, ps++)
...
bool Expand_QP(PathJob* pj, PathNode* node, const uint8_t nsizex, const uint8_t nsizey, const uint8_t sizex, const uint8_t pathoff, const Building* igb, const Unit* igu, bool roaded, uint8_t dir)
{
	...
	for(unsigned char i=0; i<DIRS; i++)
		stand[i] = Standable(npos.x + OFFSETS[i].x, npos.y + OFFSETS[i].y, nsizex, nsizey, sizex, pathoff, igb, igu, roaded);
	...
	for(unsigned char i=0; i<DIRS; i++)
	{
		...
		if(!nextn->closed && (!nextn->opened || newd < nextn->rund))
		{
			...
			nextn->rund = newd;
			//int32_t H = Magnitude( nextnpos - Vec2i(pj->goalx, pj->goaly) ) * 2;
...
#if 0
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX0, npos.y + OFFSETSY0, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 0 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX1, npos.y + OFFSETSY1, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 1 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX2, npos.y + OFFSETSY2, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 2 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX3, npos.y + OFFSETSY3, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 3 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX4, npos.y + OFFSETSY4, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 4 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX5, npos.y + OFFSETSY5, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 5 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX6, npos.y + OFFSETSY6, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 6 );
	stand |= ( ((uint8_t)Standable(npos.x + OFFSETSX7, npos.y + OFFSETSY7, nsizex, nsizey, sizex, pathoff, igb, igu, roaded) << 7 );
#endif
	...
	*/
...
#if 0
	uint8_t pass = 0;
	pass |= ( ((stand & (1<<DIR_NW)) && (stand & (1<<DIR_N)) && (stand & (1<<DIR_W))) << DIR_NW ) ;
	pass |= ( ((stand & (1<<DIR_NE)) && (stand & (1<<DIR_N)) && (stand & (1<<DIR_E))) << DIR_NE ) ;
	pass |= ( ((stand & (1<<DIR_SE)) && (stand & (1<<DIR_S)) && (stand & (1<<DIR_E))) << DIR_SE ) ;
	pass |= ( ((stand & (1<<DIR_SW)) && (stand & (1<<DIR_S)) && (stand & (1<<DIR_W))) << DIR_SW ) ;
	pass |= (stand & ( (1<<DIR_E) | (1<<DIR_N) | (1<<DIR_S) | (1<<DIR_W) ));
#endif
	...
	//for(uint8_t i=0; i<(DIRS<<1); ++i)
	{
		...
		//check pass
		bool pass = Standable(nextnpos.x, nextnpos.y, nsizex, nsizey, sizex, pathoff, igb, igu, roaded);

		if(i%2 == 1)	//diagonal?
		{
			uint8_t i2 = (i+(DIRS-1))&(DIRS-1);
			//uint8_t i2 = (i+(DIRS-1))%(DIRS);
			//pass = pass && Standable(npos.x + OFFSETS[i2].x, npos.y + OFFSETS[i2].y, nsizex, nsizey, sizex, pathoff, igb, igu, roaded);
			pass = pass && Standable(npos.x + DIRX(i2), npos.y + DIRY(i2), nsizex, nsizey, sizex, pathoff, igb, igu, roaded);
...
const Vec2s OFFSETS[DIRS] =
{
	...
	Vec2s(1, 1) //SE
		...
		PathNode* nextn;
		auto nextit = g_pathmap.find(nextni);
