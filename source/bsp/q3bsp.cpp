...
bool Q3BSP::LoadBSP(const char *name)
{
	...
	for(i = 0; i < m_numOfTextures; i++)
	{
		...
		sprintf(m_pTextures[i].strName, "tmpQuArK/textures/%s", filename);
	...
	unsigned int m_tex0vbo;
	unsigned int m_tex1vbo;
	...
	glVertexAttribPointer(s->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), &(m_pVerts[pFace->startVertIndex].vPosition));
	//glEnableClientState(GL_VERTEX_ARRAY);
	//glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glVertexAttribPointer(s->slot[SSLOT_NORMAL], 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), &(m_pVerts[pFace->startVertIndex].vPosition));
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD0], 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), &(m_pVerts[pFace->startVertIndex].vTextureCoord));
	glVertexAttribPointer(s->slot[SSLOT_TEXCOORD1], 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), &(m_pVerts[pFace->startVertIndex].vLightmapCoord));
	...
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_BYTE, &(((int*)NULL)[pFace->startIndex]));
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_BYTE, (void*)pFace->startIndex);
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_INT, &(m_pIndices[pFace->startIndex]));
#endif

	g_applog << "0.1" << std::endl;
	g_applog.flush();

#if 1
	int numi = 0;

	for (int fin = 0; fin < m_numOfFaces; ++fin)
	{
		tBSPFace* f = &m_pFaces[fin];

		if (f->type != FACE_POLYGON) continue;

		numi += f->numOfIndices;
	}

	g_applog << "0.2" << std::endl;
	g_applog.flush();

	int* ind2 = (int*)malloc(sizeof(int)*numi);
	int giin = 0;

	g_applog << "0.3" << std::endl;
	g_applog.flush();
	...
	for (int fin = 0; fin < m_numOfFaces; ++fin)
	{
		...
		for (int fiin = 0; fiin < f->numOfIndices; ++fiin, ++giin)
		{
			ind2[giin] = m_pIndices[f->startIndex + fiin] + f->startVertIndex;
		}

		f->startIndex = firstgiin;
	}
#endif
	...
	glBufferData(GL_ARRAY_BUFFER, m_numOfVerts * sizeof(tBSPVertex), m_pVerts, GL_STATIC_DRAW);
	//glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(m_pVerts + offsetof(tBSPVertex,vPosition)));	//positions
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	//glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(m_pVerts + offsetof(tBSPVertex,vNormal)));	//normals
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	//glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(m_pVerts + offsetof(tBSPVertex,vTextureCoord)));	//texc0
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	//glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(m_pVerts + offsetof(tBSPVertex,vLightmapCoord)));	//texc1 (lightmap)
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(offsetof(tBSPVertex, vPosition)));	//positions
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(offsetof(tBSPVertex, vNormal)));	//normals
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(offsetof(tBSPVertex, vTextureCoord)));	//texc0
	CHECKGLERROR();
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	CHECKGLERROR();
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(tBSPVertex), (void*)(offsetof(tBSPVertex, vLightmapCoord)));	//texc1 (lightmap)
	...
	glGenBuffers(1, &m_indvbo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indvbo);
	//glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_numOfIndices * sizeof(int), m_pIndices, GL_STATIC_DRAW);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, numi * sizeof(int), ind2, GL_STATIC_DRAW);
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indvbo);

	CHECKGLERROR();
	//glVertexAttribPointer(4, 1, GL_INT, GL_FALSE, 0, m_pIndices);
	CHECKGLERROR();
...
void Q3BSP::CheckBrush(tBSPBrush *pBrush, Vec3f vStart, Vec3f vEnd)
{
	...
	Texture* tex = &g_texture[m_textures[pBrush->textureID]];
...
void Q3BSP::RenderFace(int faceIndex)
{
...
#if 1
	Texture* tex = &g_texture[ m_textures[pFace->textureID] ];
	...
	glUniform1i(s->slot[SSLOT_LIGHTMAP], 1);

#if 0
	unsigned int nt;
	unsigned int st;

	CreateTex(nt, "tmpQuArK/textures/4lane1yl.norm.jpg", false, true);
	CreateTex(st, "tmpQuArK/textures/4lane1yl.spec.jpg", false, true);

	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, g_texture[nt].texname);
	glUniform1i(s->slot[SSLOT_LIGHTMAP], 2);

	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, g_texture[st].texname);
	glUniform1i(s->slot[SSLOT_LIGHTMAP], 3);
#endif
#endif//TODO
	...
	//glEnableClientState(GL_VERTEX_ARRAY);
	//glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glNormalPointer(GL_FLOAT, sizeof(tBSPVertex), &(m_pVerts[pFace->startVertIndex].vPosition));
	...
	//g_applog << "glde" << std::endl;
	//g_applog.flush();
	//glDrawArrays(GL_TRIANGLES, 0, 100);
	glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_INT, &(((int*)NULL)[pFace->startIndex]));
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_BYTE, NULL);
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_BYTE, (void*)pFace->startIndex);
	//glDrawElements(GL_TRIANGLES, pFace->numOfIndices, GL_UNSIGNED_INT, &(m_pIndices[pFace->startIndex]));
...
void Q3BSP::RenderLevel(const Vec3f &vPos)
{
	...
	//glBindBuffer(GL_ARRAY_BUFFER, m_posvbo);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indvbo);	//http://stackoverflow.com/questions/8973690/vao-and-element-array-buffer-state quadro driver bug
