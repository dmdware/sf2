...
void DrawBackdrop()
{
	...
	g_model[g_backdrop].Draw(0, CVector3(0,0,0), 0);
	EndVertexArrays();
}
...
bool CQuake3BSP::LoadBSP(const char *name)
{
	...
	g_backdrop = LoadModel(backdrop, CVector3(1,1,1), CVector3(0,0,0));
...
CVector3 CQuake3BSP::UnstuckNode(int nodeIndex, CVector3 vStart, CVector3 vEnd)
{
...
CVector3 CQuake3BSP::UnstuckBrush(tBSPBrush *pBrush, CVector3 vStart)
{
	...
	for(int i = 0; i < pBrush->numOfBrushSides; i++)
	{
		...
		CVector3 vInOffset = CVector3(0, 0, 0);
		CVector3 vOutOffset = CVector3(0, 0, 0);
